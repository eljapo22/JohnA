...
def create_figure(graphs, color_mapping, selected_level=None):
    ...
    # Add dashed dotted lines for specific connections in the middle path
    middle_path_file = 'combined_structure_Middle.json'
    special_nodes = ['N-000023033', 'N-000023029', 'N-000020297', 'N-000021825']
    
    if middle_path_file in graphs:
        G = graphs[middle_path_file]
            ...
            if start_node in G.nodes and end_node in G.nodes:
                try:
                    ...
                    for edge in path_edges:
                        ...
                        fig.add_trace(go.Scatter(x=[x0, x1], y=[y0, y1], mode='lines',
                                                 line=dict(width=5, color=color_mapping[middle_path_file], dash='dashdot'),
                                                 hoverinfo='text',
                                                 hovertext=f"Special Connection: {edge[0]} to {edge[1]}"))
                except nx.NetworkXNoPath:
                    print(f"No path between {start_node} and {end_node} in {middle_path_file}")
            else:
                if start_node not in G.nodes:
                    print(f"Node {start_node} not found in {middle_path_file}")
                if end_node not in G.nodes:
                    print(f"Node {end_node} not found in {middle_path_file}")
