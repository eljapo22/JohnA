...
# Function to compute the optimal path in a loop configuration
def compute_optimal_loop(graph, nodes):
    subgraph = graph.subgraph(nodes)
    mst = nx.minimum_spanning_tree(subgraph, weight='length')
    eulerian_circuit = list(nx.eulerian_circuit(nx.MultiGraph(mst)))
    optimal_path = []
    for u, v in eulerian_circuit:
        optimal_path.extend(nx.shortest_path(graph, u, v, weight='length')[:-1])
    optimal_path.append(optimal_path[0])  # Complete the loop
    return optimal_path

# Compute the optimal loop path
optimal_path_nodes = [substation_node] + all_transformer_nodes
optimal_loop_path = compute_optimal_loop(G_proj, optimal_path_nodes)
...
# Add the optimal loop path
loop_x = [G_proj.nodes[node]['x'] for node in optimal_loop_path]
loop_y = [G_proj.nodes[node]['y'] for node in optimal_loop_path]
fig.add_trace(go.Scatter(x=loop_x, y=loop_y,
                         line=dict(width=3, color='black'),  # Use black color for the loop path
                         mode='lines',
                         hoverinfo='none',
                         showlegend=True,
                         name='Optimal Loop Path'))
