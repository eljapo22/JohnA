...
def find_transformer_to_intersection_paths(G):
    transformer_intersection_paths = set()
    for node, data in G.nodes(data=True):
        if data['type'] == 'Transformer':
            for neighbor in nx.bfs_tree(G, node):
                if G.nodes[neighbor]['type'] == 'Intersection':
                    path = nx.shortest_path(G, node, neighbor)
                    for i in range(len(path) - 1):
                        transformer_intersection_paths.add((path[i], path[i+1]))
                    break
    return transformer_intersection_paths
...
def create_figure(graphs, color_mapping):
    ...
    for file_name, G in graphs.items():
        ...
        transformer_intersection_paths = find_transformer_to_intersection_paths(G)
        ...
        for edge in G.edges(data=True):
            ...
            # Check edge level and assign color
            if (start_node, end_node) in transformer_intersection_paths or (end_node, start_node) in transformer_intersection_paths:
                line_color = 'navy'
            elif edge[2].get('level') == 1:
                line_color = 'red'
            elif edge[2].get('level') == 2:
                line_color = 'green'
            else:
